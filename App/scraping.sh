...
if [[ $1 == "--monitor" ]]; then
	...
	while :
	do
		mysql -u"kaosuser" -p"kaospassword" -h"localhost" bbdd_kaos155_text -e "SELECT Type,Anyo,SUMARIO_LAST FROM lastread WHERE Type='BOE' AND stop='0';" | grep -v "SUMARIO_LAST" | awk '{ print $2 $3 }' | sed 's/BOE-S-20[0-9][0-9]/-/g' | rev | cut -c 3-10 | rev | tr "-" " " | awk '{ print $1 , "[" , $2 , "/ 12 ]  " }' | sed 's/ //g' > /tmp/boe.tmp
		clear
		for line in `cat /tmp/boe.tmp`
		do
			echo $line
		done
		sleep 2
