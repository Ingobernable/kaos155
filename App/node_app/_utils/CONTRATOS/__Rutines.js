module.exports = function(app) {
    ...
    var _this =  {
                        ...
                        if (_vx.length > 1) {
                            ...
                            if (_vx.length > 0) {
                                ...
                                if (_vx.length > 0) {
                                    ...
                                    var _vx = (value.toLowerCase().match(/\d{1,10}\.\d{2}/) || [])
                                    if (_vx.length > 0) {
                                        _imp[_imp.length] = _vx[0]
                                    } else {
                                        if (value.toLowerCase().indexOf("desierto") == -1 ) {
                                            //debugger
                                        }
                                    }
                                }
        ...
        extract: function(_arrayText, search, transforms, simple) {
            ...
            for (i in _arrayText) {
                ...
                if (_arrayText[i].toLowerCase() != null && _arrT.length==0) {
                    if (_arrayText[i].toLowerCase().indexOf(search.toLowerCase()) > -1) {
                        ...
                        if (((_arrayText[i].toLowerCase().match(/1\. /) || []).length > 0 || test) && !simple)
                            _arrayText[i] = _this.getNextPointItem(_arrayText[i], _arrayText[i], test ? (i * 1) + 1 : i) //, function (string) { return (string.match(/\d{1}\. /) || []).length > 0 })

                        if (search == 'contratista') {
                            _arrT = this.translate[search](_arrayText[i], search, transforms, simple) //[search]
                        } else {
                            _arrT = this.translate.global(_arrayText[i], search, transforms, simple) //[search](_arrayText[i], search, transforms, simple)
                            if (_arrT == null)
                                _arrT=[]
                        }
                    }
                ...
                if (!_.isArray(_arrT) && _arrT.indexOf("#") > 0 && _arrT.indexOf(";") > _arrT.indexOf("#") )
                    _arrT = _arrT.split(";")
            }
            if (!_.isArray(_arrT))
                if (_arrT.indexOf(';') > -1) {
                    ...
                    if (search.indexOf("Importe") > 0)

                        if ((_arrT.match(/lote /gi) || []).length > 0) {

                            var _p = _arrT.split(/lote/gi)

                            _.forEach(_p, function (_v) {
                                var _s = _v
                                if (_v.lastIndexOf(' Y ') > 0)
                                    _s = _.trim(_v.substr(_v.lastIndexOf(' Y ') + 3, _v.length))
                                if (_v.length > 2)
                                    _reti[_reti.length] = _s
        ...
        findImpSplitList: function (data,options,_text,_analisis, _lotes) {
            if (data.Empresa.indexOf("#") > -1) {
               ...
               var _imp = _analisis._a.importe.length > 0 ? _analisis._a.importe.replaceAll(".", "").replaceAll(",", ".") : _i
               //if (!_lotes) {
        ...
        getImporteFromString: function (_text) {
            var _arrT = []
            if (_text.indexOf("Ninguno") ==-1) {
                if (_text.indexOf("euros") > 0) {
                    if ((_text.match(/\d\,\d{3} euros/) || []).length > 0) {
                        _arrT = (_text.match(/\d{1,3}(?:\.\d{3})*,\d{3} euros/g) || [])
                    } else {
                        if ((_text.match(/\d\,\d{2} euros/) || []).length > 0) {
                            _arrT = (_text.match(/\d{1,3}(?:\.\d{3})*,\d{2} euros/g) || []) //.split("euros)")
                        } else {
                            if (_text.indexOf(") euros") > -1) {
                                if ((_text.match(/\d\,\d{2} euros/) || []).length > 0) {
                                    _arrT = (_text.replaceAll(") euros", " euros").match(/\d{1,3}(?:\.\d{3})*,\d{2} euros/g) || [])
                                } else {
                                    _arrT = (_text.replaceAll(") euros", " euros").match(/\d{1,3}(?:\.\d{3})*\ euros/g) || [])
                                }
                            } else {
                                if ((_text.match(/\d\,\d{2}/) || []).length > 0) {
                                    _arrT = (_text.match(/\d{1,3}(?:\.\d{3})*,\d{2}/g) || [])
                                } else {
                                    if ((_text.match(/\d\,\d{3}/) || []).length > 0) {
                                        _arrT = (_text.match(/\d{1,3}(?:\.\d{3})*,\d{3}/g) || [])
                                    } else {
                                        if ((_text.match(/\d{1,3}(?:\.\d{3})*\ euros/g) || []).length > 0) {
                                            _arrT = (_text.match(/\d{1,3}(?:\.\d{3})*\ euros/g) || [])
                                        } else {
                                            if ((_text.match(/\d{1,3}(?:\.\d{3})*\ €/g) || []).length > 0) {
                                                _arrT = (_text.match(/\d{1,3}(?:\.\d{3})*\ €/g) || [])
                                            } else {
                                                _arrT = []
                                                debugger
                                            }
                    ...
                    if ((_text.match(/\d{1,3}(?:\.\d{3})*,\d{2}/) || []).length > 0) {
                        _arrT = [_text.match(/\d{1,3}(?:\.\d{3})*,\d{2}/)[0]]
                    } else {
                        if ((_text.match(/\d{1,3}(?:\.\d{3})* pesetas/g) || []).length > 0) {
                            _arrT = (_text.match(/\d{1,3}(?:\.\d{3})* pesetas/g) || [])
                            for (_n in _arrT) {
                                _arrT[_n] = (_arrT[_n].replaceAll("pesetas", "").replaceAll(".", "") / 166.386).toFixed(2) * 1
                            }
                        } else {
                            if ((_text.match(/\d{1,3}(?:\.\d{3})* de pesetas/g) || []).length > 0) {
                                _arrT = (_text.match(/\d{1,3}(?:\.\d{3})* de pesetas/g) || [])
                                for (_n in _arrT) {
            ...
            if ((_.compact(_text.match(/(Importe total)/gi)) || []).length > 0) // 74.414, 34 euros.Importe total:)
                _arrT = [_arrT[_arrT.length-1]]
            return this.cleanArrayImportes(_arrT)
        },
        cleanArrayImportes: function (_arrT) {
            _ret = []
            if (_arrT.length > 0) {
                var _ret = []
    ...
    _this.translate = require('./__Extract_Data.js')(_this)
