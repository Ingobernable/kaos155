...
BEGIN
    ...
    IF _Empresa_Id>0 AND _Relacion_Id>0 THEN
		...
		SET _Tx = (SELECT JSON_OBJECT('BOLETIN', CONCAT(_BOLETIN,"/",_BOLETIN_ID) ,"Acto",_type,"texto",_key,"Anyo",_Anyo,"Mes",_Mes));
		...
		END IF;
         ...
         IF @counter >= _minRel THEN	
            call Insert_Data_PARSER_suspicius(_Relacion_Id , _MinRel);			
		 END IF;	
		
    
    else
		SET _Tx = (SELECT JSON_OBJECT('BOLETIN',CONCAT(_BOLETIN,"/",_BOLETIN_ID),"Motivo",_type,"Cargo",_key,"Anyo",_Anyo,"Mes",_Mes));
...
BEGIN
    ...
    DECLARE runners_cursor CURSOR FOR 
		SELECT borme_keys.id, _key FROM borme_relaciones JOIN borme_keys on borme_relaciones.Empresa_id=borme_keys._id WHERE borme_relations.Relation_id=_did AND NOT borme_keys.ia_suspicius;
...
-- Dump completed on 2019-04-27 21:48:12
